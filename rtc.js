...
var Web_RTC = function() {
    ...
    var errorHandler = function(err) {
        console.log(err);
    } // Fonction en charge de l'affichage des erreures rencontrées.

    var ICEServers = {
        iceServers: [{
            urls: "stun:stun.l.google.com:19302"
        }]
    }; // Serveur STUN de google. -> TODO PATCH FIREFOX url est devenu urls
    ...
    var search_P2P_Session = function(peer) {
        for (var i = 0, len = p2p_Session.length; i < len; i++)
            if (p2p_Session[i].peer == peer) return p2p_Session[i];
        return false;
    }
    ...
    this.initializePeerInstance = function(from, to, data) {
        p2p_Session.push(new peerInstance(from, to, data));
    }
    ...
    seedInstance.prototype.createInstance = function() {
        ...
        this.pc.onicecandidate = function() { // EVENT candidats recus du serveur STUN, ne peux etre déclencher que par une création d'offre ou une demande d'answer à l'offre.
            return function(ice) { // Static variable inside event. Count of candidates.
                if (ice.candidate) {
                    ...
                    if (self.seedInfo.candidates.length == 0) {
                        console.log('RTC: ICE Candidates may be already in browser cache !');
                    }
        ...
        this.pc.createOffer(function(offer) { //create an offer sdp
            self.pc.setLocalDescription(offer, function() {}, errorHandler);
    ...
    instance.prototype.set_File_Channel = function(ref) {
        ...
        this.file_Channel.onclose = function() {
            console.log('RTC: fermeture du file_channel');
        }
        ...
        this.file_Channel.onmessage = function(event) {
            if (is_chrome) {
                self.file.receavedBytes += event.data.byteLength; // -> FIREFOX
            } else {
                self.file.receavedBytes += event.data.size; // -> CHROME
            }
            ...
            if (self.file.receavedBytes >= self.file.size) {
                ...
                DOM.create_New_Window(self.file.name, false, false, false, {
                    'type': 'download',
                    'file': received,
                    'meta': self.file
                });
