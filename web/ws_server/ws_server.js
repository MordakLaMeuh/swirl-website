...
// Special msg from server, manage password, error messages... :
    ...
    client.on("message", function (str) {
        ...
        if (password) {
            if (obj.data == 'les carottes sont cuites') {
                ...
                console.log('incorrect password:' + obj.data);
            }
            return;
        }
            ...
            case 'CHAT':
                ...
                if (newClient == true) {
                    ...
                    for (var j = 0; j < clientSocket.length; j++) {
                        ...
                        // TODO EFFET DE BORD POSSIBLE, il se peut que par l'aspect assynchrone de send, la list client soit en avance.
                        inform(clientSocket[j], 'new', new Array(obj.data), 'server');
                    }
                ...
                // Normal conversation:
                    ...
                    switch (obj.data) {
                        ...
                        case "/roll":
                            ...
                            pushHistory(msg);
                            for (var j = 0; j < clientSocket.length; j++) {
                                send(clientSocket[j], "CHAT", msg, "send dice throw to " + clientUserList[j]);
                            }
                            return;
                        case "":
                            return;
                    ...
                    for (var i = 0; i < clientSocket.length; i++) {
                        if (client == clientSocket[i]) {
                            ...
                            pushHistory(msg);
                            for (var j = 0; j < clientSocket.length; j++) {
                                send(clientSocket[j], "CHAT", msg, "send msg to " + clientUserList[j]);
                            }
                            break;
                        }
                    }
