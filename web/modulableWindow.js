...
var MW = function() {
    ...
    this.create_New_Window = function(title, proportions, colors, opts, share) {
        if (!share) new Window(title, title, proportions, colors, opts);
            ...
            file_Index++;
            share.meta = (share.meta) ? share.meta : {
            ...
            new Window('id' + file_Index, share.meta.name, {
                'x': (70 - (2 * (file_Index % 5))),
                'y': (30 - (3 * (file_Index % 5))),
                'w': 30,
                'h': 70
    ...
    function switch_Css_init(class_Name, element, value, value2) {
        ...
        for (var S = 0, len = document.styleSheets.length; S < len; S++) {
            try {
                ...
                return function(invv) {
                    document.styleSheets[S].insertRule(class_Name + ' { ' + element + ': ' + ((invv) ? value2 : value) + '; }', document.styleSheets[S][cssRules].length);
                };
            } catch (err) {
                try {
                    ...
                    return function(invv) {
                        document.styleSheets[S].addRule(class_Name, element + ': ' + ((invv) ? value2 : value) + ';');
                    };
                } catch (err) {
                        ...
                        for (var R = 0; R < document.styleSheets[S][cssRules].length; R++) {
                            if (document.styleSheets[S][cssRules][R].selectorText == class_Name) {
                                if (document.styleSheets[S][cssRules][R].style[element]) {
                                    ...
                                    return function(invv) {
                                        document.styleSheets[S][cssRules][R].style[element] = ((invv) ? value2 : value);
                                    };
                                }
                            }
                        }
    ...
    var Window = function(id, title_Window, proportions, colors, opts, share) {
            ...
            this.main.onmousedown = function(e) {
                ...
                if (obj.backroom) { // TODO Patch conflit putForward car this.main.onmouse down est aussi activé quand la souris pointe sur minimize.
                    ...
                    zIndex_Max = obj.put_Forward(zIndex_Max);
                }
            ...
            var toggle_Opacity = function() {
                obj.body.style.opacity = (full_Opacity = !full_Opacity) ? 1 : obj.opacity;
            }
            ...
            obj.main.addEventListener("transitionend", function(e) {
                obj.end_Of_Transition(e);
            }, false);
    ...
    Window.prototype.media_Common = function() {
        ...
        this.media_Play_Icon.onmousedown = function() {
            obj.media.play();
        }
        this.media_Pause_Icon.onmousedown = function() {
            obj.media.pause();
        }
        this.media_Mute_Icon.onmousedown = function() {
            obj.media.volume = (obj.media.volume) ? 0 : 1;
        }
